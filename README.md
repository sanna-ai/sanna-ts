...
This is the **TypeScript implementation** of [Sanna Protocol v1.4](https://github.com/sanna-ai/sanna-protocol). For the Python reference implementation, see [sanna-ai/sanna](https://github.com/sanna-ai/sanna).

## v1.4.0 Release Notes

**Receipt Schema v1.4** — 20-field fingerprint (up from 16), adding `tool_name`, `agent_model`, `agent_model_provider`, `agent_model_version` at fingerprint positions 17-20.

**New receipt fields:**
- `tool_name` — required at cv>=9. Canonical SDK identity: `"sanna-ts"` (TS), `"sanna"` (Python). Participates in fingerprint at position 17.
- `agent_model` / `agent_model_provider` / `agent_model_version` — optional nullable fields capturing LLM model identity. Null = opt-out; absent = not captured. Fingerprint positions 18-20.

**Version changes:**
- `SPEC_VERSION` → `"1.4"`, `CHECKS_VERSION` → `"9"`, `TOOL_VERSION` → `"1.4.0"` (bare semver; SDK identity moves to `tool_name` field).
...
|-------|------|-------------|
| `spec_version` | string | Schema version, `"1.4"` |
| `tool_version` | string | Package version, e.g. `"1.4.0"` |
| `checks_version` | string | Check algorithm version, e.g. `"9"` |
...
Cross-language parity is verified against golden fixtures in the [sanna-protocol](https://github.com/sanna-ai/sanna-protocol) repository. The test suite includes cross-language verification tests covering receipt fingerprinting, content hashing, canonicalization, and signature verification. v1.4 Python fixtures (20-field fingerprint, checks_version "9") are verified end-to-end including tool_name, agent_model, enforcement_surface and invariants_scope; v1.4 TypeScript receipts use the same 20-field fingerprint formula.
