...
| `agent_identity` | object or null | Agent session identity; must include `agent_session_id`. Present only in v1.5 (cv=10) receipts. Participates in fingerprint at position 21. |
...
```json
{
  ...
  "agent_identity": {"agent_session_id": "session-a1b2c3d4e5f6"}
}
```
...
**Documentation:** [TypeScript SDK Reference](docs/ts-sdk-reference.md) — full API reference with signatures, the interceptor behavior matrix, and exact thrown types. [TypeScript SDK Cookbook](docs/cookbook.md) — delta recipes (interceptors, custom evaluators, drift, bundles, Cloud delivery).
...
Cross-language parity is verified against golden fixtures in the [sanna-protocol](https://github.com/sanna-ai/sanna-protocol) repository. The test suite includes cross-language verification tests covering receipt fingerprinting, content hashing, canonicalization, and signature verification. v1.5 Python fixtures (21-field fingerprint, checks_version "10") are verified end-to-end including tool_name, agent_model, enforcement_surface, invariants_scope, and agent_identity; v1.5 TypeScript receipts use the same 21-field fingerprint formula.
